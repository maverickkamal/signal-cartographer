...
class AetherTapScreen(Screen):
    ...
    def action_focus_spectrum(self):
        ...
        if self.aethertap_layout and self.aethertap_layout.spectrum_pane:
            self.aethertap_layout.spectrum_pane.focus()
            if self.aethertap_layout.log_pane:
                self.aethertap_layout.log_pane.add_log_entry("Focused on Main Spectrum Analyzer [MSA]")
    
    def action_focus_signal(self):
        """Focus on the signal focus pane (F2)"""
        if self.aethertap_layout and self.aethertap_layout.signal_focus_pane:
            self.aethertap_layout.signal_focus_pane.focus()
            if self.aethertap_layout.log_pane:
                self.aethertap_layout.log_pane.add_log_entry("Focused on Signal Focus & Data [SFD]")
    
    def action_focus_map(self):
        """Focus on the cartography pane (F3)"""
        if self.aethertap_layout and self.aethertap_layout.cartography_pane:
            self.aethertap_layout.cartography_pane.focus()
            if self.aethertap_layout.log_pane:
                self.aethertap_layout.log_pane.add_log_entry("Focused on Cartography & Navigation [CNP]")
    
    def action_focus_decoder(self):
        """Focus on the decoder pane (F4)"""
        if self.aethertap_layout and self.aethertap_layout.decoder_pane:
            self.aethertap_layout.decoder_pane.focus()
            if self.aethertap_layout.log_pane:
                self.aethertap_layout.log_pane.add_log_entry("Focused on Decoder & Analysis Toolkit [DAT]")
    
    def action_focus_log(self):
        """Focus on the log pane (F5)"""
        if self.aethertap_layout and self.aethertap_layout.log_pane:
            self.aethertap_layout.log_pane.focus()
            if self.aethertap_layout.log_pane:
                self.aethertap_layout.log_pane.add_log_entry("Focused on Captain's Log & Database [CLD]")
    ...
    async def _initialize_panes(self):
        ...
        if self.aethertap_layout:
            ...
            if self.aethertap_layout.log_pane:
                # Check if content_widget is available
                if hasattr(self.aethertap_layout.log_pane, 'content_widget') and self.aethertap_layout.log_pane.content_widget:
                    startup_messages = [
                        "=" * 60,
                    ...
                    # Debug: content_widget not ready yet
                    print(f"DEBUG: Log pane content_widget not ready. Has content_widget: {hasattr(self.aethertap_layout.log_pane, 'content_widget')}")
                    if hasattr(self.aethertap_layout.log_pane, 'content_widget'):
                        print(f"DEBUG: content_widget value: {self.aethertap_layout.log_pane.content_widget}")
            else:
                print("DEBUG: log_pane not available")
            ...
            if self.aethertap_layout.spectrum_pane:
                ...
                # Add some immediate debug content
                if hasattr(self.aethertap_layout.spectrum_pane, 'content_widget') and self.aethertap_layout.spectrum_pane.content_widget:
                    self.aethertap_layout.spectrum_pane.content_widget.write("Spectrum analyzer ready...")
                else:
                    print("DEBUG: Spectrum pane content_widget not ready")
            
            # Initialize signal focus pane
            if self.aethertap_layout.signal_focus_pane:
                if hasattr(self.aethertap_layout.signal_focus_pane, 'content_widget') and self.aethertap_layout.signal_focus_pane.content_widget:
                    self.aethertap_layout.signal_focus_pane.content_widget.write("No signal focused")
                else:
                    print("DEBUG: Signal focus pane content_widget not ready")
